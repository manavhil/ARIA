...
import { ArrowRight, Zap, Languages } from 'lucide-react';
...
const content = {
    ...
    ar: {
        eyebrow: "محرك الرندر الديناميكي",
        title: "أثير فلو",
        description: "إطار عمل ذكي ومتكيف لإنشاء تجارب رقمية سلسة تشعر بالحياة وتستجيب لتفاعل المستخدم في الوقت الفعلي.",
        cta: "استكشف المحرك",
        langName: "English"
    }
}
...
const AetherFlowHero = ({ lang, toggleLang }: AetherFlowHeroProps) => {
    ...
    return (
        <div className={`relative h-screen w-full flex flex-col items-center justify-center overflow-hidden ${isAr ? 'font-arabic' : 'font-sans'}`} dir={isAr ? 'rtl' : 'ltr'}>
            ...
            <button 
                onClick={toggleLang}
                className="fixed top-8 end-8 z-50 flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 rounded-full transition-all text-sm text-neutral-400 hover:text-white backdrop-blur-md cursor-pointer"
            >
                <Languages className="w-4 h-4" />
                <span>{content[lang].langName}</span>
            </button>
            ...
            <div className="relative z-10 text-center p-6">
                ...
                <motion.h1
                    ...
                    initial="hidden"
                    animate="visible"
                    className={`text-5xl md:text-8xl font-bold tracking-tighter mb-6 bg-clip-text text-transparent bg-gradient-to-b from-white to-gray-400 ${isAr ? 'tracking-normal' : ''}`}
                >
                    {content[lang].title}
                </motion.h1>
