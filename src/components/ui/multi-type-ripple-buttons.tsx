...
export const RippleButton: React.FC<RippleButtonProps> = ({
  ...
  rippleDuration = 600,
  hoverBaseColor = '#f97316', // Updated to Tailwind orange-500
  hoverRippleColor: customHoverRippleColor,
  hoverBorderEffectColor = '#f9731677', // Updated to orange theme
  ...
  const determinedJsRippleColor = useMemo(() => {
    ...
    return 'var(--button-ripple-color, rgba(249, 115, 22, 0.3))'; // Default to subtle orange
  ...
  const createJsRipple = (event: MouseEvent<HTMLButtonElement>) => {
    ...
    setTimeout(() => setJsRipples(current => current.filter(r => r.key !== newRipple.key)), rippleDuration);
  };
  ...
  const baseClasses = "relative border-none overflow-hidden isolate transition-all duration-200 cursor-pointer px-4 py-2 bg-orange-600 hover:opacity-90 text-white rounded-lg";
  return (
    <>
      <style dangerouslySetInnerHTML={{ __html: JS_RIPPLE_KEYFRAMES }} />
      <button className={`${baseClasses} ${disabled ? "opacity-50" : ""} ${className}`} onClick={handleButtonClick} disabled={disabled}>
